...
void base_multiplication_layer(int64_t *result_t, int64_t *at, int64_t *bt, const int layer){
	if (layer == 7){
		for (int i = 0; i < SABER_N; i += 4){
			...
			int64_t result_low[4] = {0}, result_high[4] = {0};

#ifdef KARATSUBA
			my_karatsuba(at+i, bt+i, result_low, 2);
		    for (int j = 2; j < 4; j++){
		    	result_t[i+j-2] = (result_low[j-2] + (result_low[j] % Q) * xi) % Q;
		    }
		    my_karatsuba(at+i+2, bt+i+2, result_high, 2);
		    for (int j = 2; j < 4; j++){
		    	result_t[i+j] = (result_high[j-2] - (result_high[j] % Q) * xi) % Q;
		    }
#else
